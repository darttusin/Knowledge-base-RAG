...
`input`  must be a tensor with at least 2 dimensions.
...
`bins`  can be a sequence of N 1D tensors, a sequence of N ints, or a single int. 

If `bins`  is a sequence of N 1D tensors, it explicitly specifies the N sequences
...
corresponding dimension. The [`range`](torch.range.html#torch.range "torch.range")  argument can be provided to manually
...
See also [`torch.histogram()`](torch.histogram.html#torch.histogram "torch.histogram")  , which specifically computes 1D histograms.
While [`torch.histogramdd()`](#torch.histogramdd "torch.histogramdd")  infers the dimensionality of its bins and
binned values from the shape of `input`  , [`torch.histogram()`](torch.histogram.html#torch.histogram "torch.histogram")  accepts and flattens `input`  of any shape.
...
Keyword Arguments
:   * **range** ( *sequence* *of* [*float*](https://docs.python.org/3/library/functions.html#float "(in Python v3.13)")  ) – Defines the leftmost and rightmost bin edges
in each dimension.
* **weight** ( [*Tensor*](../tensors.html#torch.Tensor "torch.Tensor")  ) – By default, each value in the input has weight 1. If a weight
tensor is passed, each N-dimensional coordinate in input
contributes its associated weight towards its bin’s result.
The weight tensor should have the same shape as the `input`  tensor excluding its innermost dimension N.
* **density** ( [*bool*](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)")  ) – If False (default), the result will contain the count (or total weight)
