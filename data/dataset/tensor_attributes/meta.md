...
perform data-dependent operations like [`torch.nonzero()`](generated/torch.nonzero.html#torch.nonzero "torch.nonzero")  or [`item()`](generated/torch.Tensor.item.html#torch.Tensor.item "torch.Tensor.item")  . In some cases, not all device types (e.g., CPU
...
An object can be loaded with [`torch.load()`](generated/torch.load.html#torch.load "torch.load")  onto meta device by specifying `map_location='meta'`  : 
...
the [`torch.device()`](tensor_attributes.html#torch.device "torch.device")  context manager: 
...
Use a factory function like [`torch.empty_like()`](generated/torch.empty_like.html#torch.empty_like "torch.empty_like")  to explicitly specify how
you would like the missing data to be filled in. 

NN modules have a convenience method [`torch.nn.Module.to_empty()`](generated/torch.nn.Module.html#torch.nn.Module.to_empty "torch.nn.Module.to_empty")  that
