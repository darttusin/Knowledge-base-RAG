...
torch. vmap ( *func*  , *in_dims = 0*  , *out_dims = 0*  , *randomness = 'error'*  , *** , *chunk_size = None* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_functorch/apis.py#L30) 
...
[`torch.vmap()`](#torch.vmap "torch.vmap")  is aliased to [`torch.func.vmap()`](torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  for
...
One example of using [`vmap()`](#torch.vmap "torch.vmap")  is to compute batched dot products. PyTorch
doesn’t provide a batched `torch.dot`  API; instead of unsuccessfully
rummaging through docs, use [`vmap()`](#torch.vmap "torch.vmap")  to construct a new function. 
...
[`vmap()`](#torch.vmap "torch.vmap")  can be helpful in hiding batch dimensions, leading to a simpler
...
[`vmap()`](#torch.vmap "torch.vmap")  can also help vectorize computations that were previously difficult
or impossible to batch. One example is higher-order gradient computation.
The PyTorch autograd engine computes vjps (vector-Jacobian products).
Computing a full Jacobian matrix for some function f: R^N -> R^N usually
requires N calls to `autograd.grad`  , one per Jacobian row. Using [`vmap()`](#torch.vmap "torch.vmap")  ,
...
[`vmap()`](#torch.vmap "torch.vmap")  can also be nested, producing an output with multiple batched dimensions 
