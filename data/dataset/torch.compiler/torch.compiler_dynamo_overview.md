...
Before you read this section, read [torch.compiler](torch.compiler.html#torch-compiler-overview)  . 
...
operations into an [FX Graph](https://localhost:8000/docs/stable/fx.html)  which is then compiled with a customizable backend.
...
backends to make PyTorch code faster with a single line decorator `torch._dynamo.optimize()`  which is wrapped for convenience by `torch.compile()` 

The following diagram demonstrates how PyTorch works with `torch.compile`  and without it: 

![_images/TorchDynamo.png](_images/TorchDynamo.png)

`TorchInductor`  is one of the backends
supported by [Dynamo Graph](https://localhost:8000/docs/stable/fx.html)  into [Triton](https://github.com/openai/triton)  for GPUs or [C++/OpenMP](https://www.openmp.org/)  for CPUs. We have a [training performance dashboard](https://github.com/pytorch/torchdynamo/issues/681#issuecomment-1233828468)  that provides performance comparison for different training backends. You can read
more in the [TorchInductor post on PyTorch
dev-discuss](https://dev-discuss.localhost:8000/t/torchinductor-a-pytorch-native-compiler-with-define-by-run-ir-and-symbolic-shapes/747)  . 
...
* [dev-discuss topics](https://dev-discuss.localhost:8000/search?q=TorchDynamo%20order%3Alatest)
...
tool is [depyf](https://github.com/youkaichao/depyf)  . If you don’t have `depyf`  already installed, run `pip install depyf`  . Then, add the
...
In the modified bytecode, `__compiled_fn_0`  is the return value of `my_compiler()`  (the compiled graph). `__resume_at_30_1`  and `__resume_at_38_2`  are both generated continuation functions that pick
...
![_images/flowchart.jpg](_images/flowchart.jpg)

To learn more about how all this is implemented internally, see [Dynamo Deep-Dive](torch.compiler_dynamo_deepdive.html#torch-compiler-dynamo-deepdive)  .
