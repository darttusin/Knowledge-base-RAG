...
TorchElastic agents use [`torch.distributed.elastic.multiprocessing.start_processes()`](multiprocessing.html#torch.distributed.elastic.multiprocessing.start_processes "torch.distributed.elastic.multiprocessing.start_processes")  to launch the workers which has a simple file based inter-process error propagation
built-in. 

Any function or binary entrypoint decorated with [`record()`](#torch.distributed.elastic.multiprocessing.errors.record "torch.distributed.elastic.multiprocessing.errors.record")  will write uncaught exceptions (with the trace information) to a file specified by the
...
torch.distributed.elastic.multiprocessing.errors. record ( *fn*  , *error_handler = None* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/distributed/elastic/multiprocessing/errors/__init__.py#L309) 
...
Return type
:   [*Callable*](https://docs.python.org/3/library/typing.html#typing.Callable "(in Python v3.13)")  [[~_P], [*Optional*](https://docs.python.org/3/library/typing.html#typing.Optional "(in Python v3.13)")  [ *_R*  ]]

*class* torch.distributed.elastic.multiprocessing.errors. ChildFailedError ( *name*  , *failures* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/distributed/elastic/multiprocessing/errors/__init__.py#L200) 
:   Special exception type that can be raised from a function annotated with the `@record`  decorator to have the child process’ (root exception) propagate
...
The torchelastic agent raises a `ChildFailedError("trainer", {1: "trainer_1/error.json"})`  upon detecting trainer 1’s failure which would propagate the contents
of trainer 1’s error file to the scheduler’s init process.

*class* torch.distributed.elastic.multiprocessing.errors. ErrorHandler [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/distributed/elastic/multiprocessing/errors/error_handler.py#L24) 
...
*class* torch.distributed.elastic.multiprocessing.errors. ProcessFailure ( *local_rank*  , *pid*  , *exitcode*  , *error_file* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/distributed/elastic/multiprocessing/errors/__init__.py#L90) 
...
It is assumed that the `error_file`  is written by `torch.distributed.elastic.multiprocessing.errors.error_handler.ErrorHandler`  .
