...
torch. std ( *input*  , *dim = None*  , *** , *correction = 1*  , *keepdim = False*  , *out = None* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
:   Calculates the standard deviation over the dimensions specified by `dim`  . `dim`  can be a single dimension, list of dimensions, or `None`  to
