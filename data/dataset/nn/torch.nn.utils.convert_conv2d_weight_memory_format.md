...
torch.nn.utils. convert_conv2d_weight_memory_format ( *module*  , *memory_format* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/utils/memory_format.py#L11) 
...
permutation to an earlier layer, as most layers are quite agnostic to `memory_format`  . 
...
Parameters
:   * **module** ( [*nn.Module*](torch.nn.Module.html#torch.nn.Module "torch.nn.Module")  ) – `nn.Conv2d`  & `nn.ConvTranspose2d`  or container `nn.Module`
* **memory_format** ( [*memory_format*](../tensor_attributes.html#torch.memory_format "torch.memory_format")  ) – user specified `memory_format`  ,
