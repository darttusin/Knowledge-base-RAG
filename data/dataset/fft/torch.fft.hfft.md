...
torch.fft. hfft ( *input*  , *n = None*  , *dim = -1*  , *norm = None*  , *** , *out = None* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
:   Computes the one dimensional discrete Fourier transform of a Hermitian
symmetric `input`  signal. 

Note 

[`hfft()`](#torch.fft.hfft "torch.fft.hfft")  / [`ihfft()`](torch.fft.ihfft.html#torch.fft.ihfft "torch.fft.ihfft")  are analogous to [`rfft()`](torch.fft.rfft.html#torch.fft.rfft "torch.fft.rfft")  / [`irfft()`](torch.fft.irfft.html#torch.fft.irfft "torch.fft.irfft")  . The real FFT expects
...
same way as with [`irfft()`](torch.fft.irfft.html#torch.fft.irfft "torch.fft.irfft")  .
...
component will be ignored. For example, any imaginary component in `input[0]`  would result in one or more complex frequency terms which
...
* **dim** ( [*int*](https://docs.python.org/3/library/functions.html#int "(in Python v3.13)") *,* *optional*  ) – The dimension along which to take the one dimensional Hermitian FFT.
* **norm** ( [*str*](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)") *,* *optional*  ) –
    ...
    Normalization mode. For the forward transform
        ( [`hfft()`](#torch.fft.hfft "torch.fft.hfft")  ), these correspond to:

    + `"forward"`  - normalize by `1/n`
        + `"backward"`  - no normalization
        + `"ortho"`  - normalize by `1/sqrt(n)`  (making the Hermitian FFT orthonormal)
        Calling the backward transform ( [`ihfft()`](torch.fft.ihfft.html#torch.fft.ihfft "torch.fft.ihfft")  ) with the same
        normalization mode will apply an overall normalization of `1/n`  between
        the two transforms. This is required to make [`ihfft()`](torch.fft.ihfft.html#torch.fft.ihfft "torch.fft.ihfft")  the exact inverse.
...
Like with [`irfft()`](torch.fft.irfft.html#torch.fft.irfft "torch.fft.irfft")  , the output length must be given in order
