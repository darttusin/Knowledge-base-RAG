...
*class* torch.nn. ConvTranspose2d ( *in_channels*  , *out_channels*  , *kernel_size*  , *stride = 1*  , *padding = 0*  , *output_padding = 0*  , *groups = 1*  , *bias = True*  , *dilation = 1*  , *padding_mode = 'zeros'*  , *device = None*  , *dtype = None* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/modules/conv.py#L976) 
...
* `stride`  controls the stride for the cross-correlation. When stride > 1, ConvTranspose2d inserts zeros between input
...
when a [`Conv2d`](torch.nn.Conv2d.html#torch.nn.Conv2d "torch.nn.Conv2d")  and a [`ConvTranspose2d`](#torch.nn.ConvTranspose2d "torch.nn.ConvTranspose2d")  are initialized with same parameters, they are inverses of each other in
regard to the input and output shapes. However, when `stride > 1`  , [`Conv2d`](torch.nn.Conv2d.html#torch.nn.Conv2d "torch.nn.Conv2d")  maps multiple input shapes to the same output
...
</math> -->( N , C i n , H i n , W i n ) (N, C_{in}, H_{in}, W_{in})( N , C in ​ , H in ​ , W in ​ )  or <!-- MathML: <math xmlns="http://www.w3.org/1998/Math/MathML">
<semantics>
<mrow>
<mo stretchy="false">
                (
...
</math> -->( C i n , H i n , W i n ) (C_{in}, H_{in}, W_{in})( C in ​ , H in ​ , W in ​ )
...
</math> -->( N , C o u t , H o u t , W o u t ) (N, C_{out}, H_{out}, W_{out})( N , C o u t ​ , H o u t ​ , W o u t ​ )  or <!-- MathML: <math xmlns="http://www.w3.org/1998/Math/MathML">
<semantics>
<mrow>
<mo stretchy="false">
                (
...
<annotation encoding="application/x-tex">
             H_{out} = (H_{in} - 1) times text{stride}[0] - 2 times text{padding}[0] + text{dilation}[0]
          times (text{kernel_size}[0] - 1) + text{output_padding}[0] + 1
            </annotation>
</semantics>
</math> -->
H o u t = ( H i n − 1 ) × stride [ 0 ] − 2 × padding [ 0 ] + dilation [ 0 ] × ( kernel_size [ 0 ] − 1 ) + output_padding [ 0 ] + 1 H_{out} = (H_{in} - 1) times text{stride}[0] - 2 times text{padding}[0] + text{dilation}[0]
 times (text{kernel_size}[0] - 1) + text{output_padding}[0] + 1

H o u t ​ = ( H in ​ − 1 ) × stride [ 0 ] − 2 × padding [ 0 ] + dilation [ 0 ] × ( kernel_size [ 0 ] − 1 ) + output_padding [ 0 ] + 1
...
<annotation encoding="application/x-tex">
             W_{out} = (W_{in} - 1) times text{stride}[1] - 2 times text{padding}[1] + text{dilation}[1]
          times (text{kernel_size}[1] - 1) + text{output_padding}[1] + 1
            </annotation>
</semantics>
</math> -->
W o u t = ( W i n − 1 ) × stride [ 1 ] − 2 × padding [ 1 ] + dilation [ 1 ] × ( kernel_size [ 1 ] − 1 ) + output_padding [ 1 ] + 1 W_{out} = (W_{in} - 1) times text{stride}[1] - 2 times text{padding}[1] + text{dilation}[1]
 times (text{kernel_size}[1] - 1) + text{output_padding}[1] + 1

W o u t ​ = ( W in ​ − 1 ) × stride [ 1 ] − 2 × padding [ 1 ] + dilation [ 1 ] × ( kernel_size [ 1 ] − 1 ) + output_padding [ 1 ] + 1
...
</math> -->kernel_size[0] , kernel_size[1] ) text{kernel_size[0]}, text{kernel_size[1]})kernel_size[0] , kernel_size[1] )  .
...
<annotation encoding="application/x-tex">
               k = frac{groups}{C_text{out} * prod_{i=0}^{1}text{kernel_size}[i]}
              </annotation>
</semantics>
</math> -->k = g r o u p s C out ∗ ∏ i = 0 1 kernel_size [ i ] k = frac{groups}{C_text{out} * prod_{i=0}^{1}text{kernel_size}[i]}k = C out ​ ∗ ∏ i = 0 1 ​ kernel_size [ i ] g ro u p s ​
...
forward ( *input*  , *output_size = None* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/modules/conv.py#L1133) 
:   Performs the forward pass. 

Variables
:   * **input** ( [*Tensor*](../tensors.html#torch.Tensor "torch.Tensor")  ) – The input tensor.
* **output_size** ( [*list*](https://docs.python.org/3/library/stdtypes.html#list "(in Python v3.13)") *[* [*int*](https://docs.python.org/3/library/functions.html#int "(in Python v3.13)") *]* *,* *optional*  ) – A list of integers representing
