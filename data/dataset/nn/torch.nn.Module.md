...
*class* torch.nn. Module ( ** args*  , *** kwargs* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/modules/module.py#L404) 
...
parameters converted when you call [`to()`](#torch.nn.Module.to "torch.nn.Module.to")  , etc. 
...
Variables
: **training** ( [*bool*](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)")  ) – Boolean represents whether this module is in training or
evaluation mode.
...
Return type
:   [*Iterator*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Iterator "(in Python v3.13)")  [ [*Module*](#torch.nn.Module "torch.nn.modules.module.Module")  ]
...
mode, i.e. whether they are affected, e.g. [`Dropout`](torch.nn.Dropout.html#torch.nn.Dropout "torch.nn.Dropout")  , `BatchNorm`  ,
etc. 
...
forward ( ** input* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/modules/module.py#L388) 
...
this function, one should call the [`Module`](#torch.nn.Module "torch.nn.Module")  instance afterwards
...
Implement this and a corresponding [`set_extra_state()`](#torch.nn.Module.set_extra_state "torch.nn.Module.set_extra_state")  for your module
...
:   Copy parameters and buffers from [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  into this module and its descendants. 

If `strict`  is `True`  , then
the keys of [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  must exactly match the keys returned
by this module’s [`state_dict()`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  function. 

Warning 

If `assign`  is `True`  the optimizer must be created after
the call to [`load_state_dict`](#torch.nn.Module.load_state_dict "torch.nn.Module.load_state_dict")  unless [`get_swap_module_params_on_conversion()`](../future_mod.html#torch.__future__.get_swap_module_params_on_conversion "torch.__future__.get_swap_module_params_on_conversion")  is `True`  .
...
in [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  match the keys returned by this module’s [`state_dict()`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  function. Default: `True`
...
exists in [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  , [`load_state_dict()`](#torch.nn.Module.load_state_dict "torch.nn.Module.load_state_dict")  will raise a `RuntimeError`  .
...
Parameters
:   * **memo** ( [*Optional*](https://docs.python.org/3/library/typing.html#typing.Optional "(in Python v3.13)") *[* [*set*](https://docs.python.org/3/library/stdtypes.html#set "(in Python v3.13)") *[* [*torch.nn.modules.module.Module*](#torch.nn.Module "torch.nn.modules.module.Module") *]* *]*  ) – a memo to store the set of modules already added to the result
...
This function is deprecated in favor of [`register_full_backward_hook()`](#torch.nn.Module.register_full_backward_hook "torch.nn.Module.register_full_backward_hook")  and
...
is that the latter will not be a part of this module’s [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  . 
...
that run on buffers, such as [`cuda`](#torch.nn.Module.cuda "torch.nn.Module.cuda")  , are ignored. If `None`  ,
the buffer is **not** included in the module’s [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  .
* **persistent** ( [*bool*](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)")  ) – whether the buffer is part of this module’s [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  .
...
The hook will be called every time after [`forward()`](#torch.nn.Module.forward "torch.nn.Module.forward")  has computed an output. 

If `with_kwargs`  is `False`  or not specified, the input contains only
the positional arguments given to the module. Keyword arguments won’t be
passed to the hooks and only to the `forward`  . The hook can modify the
output. It can modify the input inplace but it will not have effect on
forward since this is called after [`forward()`](#torch.nn.Module.forward "torch.nn.Module.forward")  is called. The hook
...
before all existing `forward`  hooks on this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Otherwise, the provided `hook`  will be fired after all existing `forward`  hooks on
this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Note that global `forward`  hooks registered with `register_module_forward_hook()`  will fire before all hooks
...
The hook will be called every time before [`forward()`](#torch.nn.Module.forward "torch.nn.Module.forward")  is invoked. 
...
all existing `forward_pre`  hooks on this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Otherwise, the provided `hook`  will be fired after all existing `forward_pre`  hooks
on this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Note that global `forward_pre`  hooks registered with `register_module_forward_pre_hook()`  will fire before all
...
:   * **hook** ( *Callable*  ) – The user-defined hook to be registered.
* **prepend** ( [*bool*](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)")  ) – If true, the provided `hook`  will be fired before
all existing `backward`  hooks on this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Otherwise, the provided `hook`  will be fired after all existing `backward`  hooks on
this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Note that global `backward`  hooks registered with `register_module_full_backward_hook()`  will fire before
...
:   * **hook** ( *Callable*  ) – The user-defined hook to be registered.
* **prepend** ( [*bool*](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)")  ) – If true, the provided `hook`  will be fired before
all existing `backward_pre`  hooks on this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Otherwise, the provided `hook`  will be fired after all existing `backward_pre`  hooks
on this [`torch.nn.Module`](#torch.nn.Module "torch.nn.Module")  . Note that global `backward_pre`  hooks registered with `register_module_full_backward_pre_hook()`  will fire before
all hooks registered by this method.
...
Note that the checks performed when calling [`load_state_dict()`](#torch.nn.Module.load_state_dict "torch.nn.Module.load_state_dict")  with `strict=True`  are affected by modifications the hook makes to `missing_keys`  or `unexpected_keys`  , as expected. Additions to either
...
:   Alias for [`add_module()`](#torch.nn.Module.add_module "torch.nn.Module.add_module")  .
...
from this module using the given name
* **param** ( [*Parameter*](torch.nn.parameter.Parameter.html#torch.nn.parameter.Parameter "torch.nn.parameter.Parameter") *or* *None*  ) – parameter to be added to the module. If `None`  , then operations that run on parameters, such as [`cuda`](#torch.nn.Module.cuda "torch.nn.Module.cuda")  ,
are ignored. If `None`  , the parameter is **not** included in the
module’s [`state_dict`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  .

register_state_dict_post_hook ( *hook* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/modules/module.py#L2098) 
:   Register a post-hook for the [`state_dict()`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  method. 
...
:   Register a pre-hook for the [`state_dict()`](#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  method. 
...
This function is called from [`load_state_dict()`](#torch.nn.Module.load_state_dict "torch.nn.Module.load_state_dict")  to handle any extra state
found within the *state_dict* . Implement this function and a corresponding [`get_extra_state()`](#torch.nn.Module.get_extra_state "torch.nn.Module.get_extra_state")  for your module if you need to store extra state within its *state_dict* . 
...
to look for. (See above example for how to specify a
fully-qualified string.)
* **module** ( [*Module*](#torch.nn.Module "torch.nn.modules.module.Module")  ) – The module to set the submodule to.
...
state_dict ( *** , *destination : T_destination*  , *prefix : [str](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)") = ''*  , *keep_vars : [bool](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)") = False* ) → T_destination [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/modules/module.py#L2186) 
state_dict ( *** , *prefix : [str](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)") = ''*  , *keep_vars : [bool](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)") = False* ) → [dict](https://docs.python.org/3/library/stdtypes.html#dict "(in Python v3.13)") [ [str](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)") , Any ]
...
to ( *device : Optional [ DeviceLikeType ] = ...*  , *dtype : Optional [ [dtype](../tensor_attributes.html#torch.dtype "torch.dtype") ] = ...*  , *non_blocking : [bool](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)") = ...* ) → Self [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/nn/modules/module.py#L1242) 
to ( *dtype : [dtype](../tensor_attributes.html#torch.dtype "torch.dtype")*  , *non_blocking : [bool](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)") = ...* ) → Self
to ( *tensor : [Tensor](../tensors.html#torch.Tensor "torch.Tensor")*  , *non_blocking : [bool](https://docs.python.org/3/library/functions.html#bool "(in Python v3.13)") = ...* ) → Self
