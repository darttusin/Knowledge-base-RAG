...
Before you read this section, make sure to read the [torch.compiler](torch.compiler.html#torch-compiler-overview) 

let’s start by looking at a simple `torch.compile`  example that demonstrates
how to use `torch.compile`  for inference. This example demonstrates the `torch.cos()`  and `torch.sin()`  features which are examples of pointwise
...
in the snippet below and it will run on CPU. You can also set device to `xpu:0`  to run on Intel® GPUs.
...
Read more on Triton’s performance [here](https://openai.com/blog/triton/)  . Because the code is written
...
PyTorch users frequently leverage pretrained models from [transformers](https://github.com/huggingface/transformers)  or [TIMM](https://github.com/rwightman/pytorch-image-models)  and one of
...
If you remove the `to(device="cuda:0")`  from the model and `encoded_input`  , then Triton will generate C++ kernels that will be
...
* [torch.compile tutorial on training](https://localhost:8000/tutorials/intermediate/torch_compile_tutorial.html)
* [torch.compiler API reference](torch.compiler_api.html#torch-compiler-api)
* [TorchDynamo APIs for fine-grained tracing](torch.compiler_fine_grain_apis.html#torchdynamo-fine-grain-tracing)
