...
For a longer background on CUDAGraphs, read [accelerating pytorch with CUDAGraphs](https://localhost:8000/blog/accelerating-pytorch-with-cuda-graphs/)  . 
...
PyTorch provides a [convenience wrapper](https://localhost:8000/docs/stable/generated/torch.cuda.CUDAGraph.html)  around CUDAGraphs that handles a couple of tricky interactions with PyTorch’s caching allocator. 
...
> **Note** 
>  Enable input mutation support by setting [torch._inductor.config.cudagraph_support_input_mutation = True](https://github.com/pytorch/pytorch/blob/main/torch/_inductor/config.py#L662)  for “reduce-overhead” mode.

### Dynamic Shape Support 

[Dynamic shape](https://localhost:8000/docs/stable/torch.compiler_dynamic_shapes.html)  means that an input tensor has different shapes across function calls. Since CUDAGraph
...
setting [torch._inductor.config.triton.cudagraph_skip_dynamic_graphs=True](https://github.com/pytorch/pytorch/blob/main/torch/_inductor/config.py#L653)  allows to skip cudagraphing functions with dynamic shape inputs and only cudagraphing
...
libraries such as NCCL for cross-device communication. Please see *NCCL Support*  section for more details.
* **Free unbacked symbols** : Free unbacked symbols usually happen during [dynamic shapes](https://localhost:8000/docs/stable/torch.compiler_dynamic_shapes.html)  .
...
The following operators are incompatible when [torch.are_deterministic_algorithms_enabled()](https://localhost:8000/docs/stable/generated/torch.are_deterministic_algorithms_enabled.html)  . 
...
are wrong, you can mark the start of a new iteration with [torch.compiler.mark_step_begin()](https://localhost:8000/docs/stable/generated/torch.compiler.cudagraph_mark_step_begin.html)  , or clone
