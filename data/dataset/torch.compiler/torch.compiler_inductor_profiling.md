...
* `TORCHINDUCTOR_UNIQUE_KERNEL_NAMES`
    ...
    + By default, TorchInductor names a Triton kernel as `‘triton_’`  . When
        this environmental variable is enabled, inductor generates a more
        meaningful kernel name in the trace, for example, `triton_poi_fused_cat_155`  which contains the kernel category
...
1. Run the benchmark script for the model:
    ...
    The tool relies on kernel name to decide its category. Enabling `TORCHINDUCTOR_UNIQUE_KERNEL_NAMES`  is crucial for that.
...
I’ll name it `fwd.py`  for convenience. Run it directly with the `-p`  argument:
 ...
 **Chrome trace for the profile is written to /tmp/compiled_module_profile.json**
    ...
    ![_images/trace.png](_images/trace.png)
...
let’s check reduction kernels:
    ...
    ![_images/kernel_breakdown.png](_images/kernel_breakdown.png)
...
Let’s say we want to take a closer look at `triton_red_fused__native_batch_norm_legit_functional_16`  which is the
...
![_images/inductor_code.png](_images/inductor_code.png)
...
![_images/terminal_printout.png](_images/terminal_printout.png)
