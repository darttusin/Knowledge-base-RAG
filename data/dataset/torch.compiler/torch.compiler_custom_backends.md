...
The returned callable should have the same contract as the `forward`  function of the original `torch.fx.GraphModule`  passed into the backend: `(*args: torch.Tensor) -> List[torch.Tensor]`  . 

In order for TorchDynamo to call your backend, pass your backend function as the `backend`  kwarg in `torch.compile`  . For example, 
...
You can learn more about `entry_points`  in the [python packaging documentation](https://setuptools.pypa.io/en/latest/userguide/entry_point.html)  .

To register your backend through `entry_points`  , you could add your backend function to the `torch_dynamo_backends`  entry point group in the `setup.py`  file of your package like: 
...
When you call `torch.compile(model, backend="my_compiler")`  , PyTorch would first search the backend named `my_compiler`  that has been registered with `register_backend`  . If not found, it will continue to search in all backends registered
via `entry_points`  . 
...
* It is required for use with the [minifier](https://localhost:8000/docs/main/torch.compiler_troubleshooting_old.html#minifier)  . Any generated
...
* AOTAutograd produces FX graphs consisting of [core Aten ops](https://localhost:8000/docs/main/torch.compiler_ir.html#core-aten-ir)  . As a result,
...
Wrap your backend with `torch._dynamo.backends.common.aot_autograd`  and use `torch.compile`  with the `backend`  kwarg as before.
...
backend in this section, that will print pretty print the fx `GraphModule`  extracted from Dynamo’s bytecode analysis
...
with [optimize_for_inference](https://localhost:8000/docs/stable/generated/torch.jit.optimize_for_inference.html)  : 
...
TorchDynamo includes many backends, which can be listed with `torch._dynamo.list_backends()`  . You can combine these backends
