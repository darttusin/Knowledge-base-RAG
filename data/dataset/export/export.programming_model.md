...
This document aims to explain the behaviors and capabilities of [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  . It is intended to help build your intuition
for how [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  handles code. 

Basics of Tracing 
----------------------------------------------------------------------

[`torch.export.export()`](export.html#torch.export.export "torch.export.export")  captures a graph representing your model by
...
The basic output of [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  is a single graph of PyTorch
operations, with associated metadata. The exact format of this output is
covered in the [torch.export IR Specification](export.ir_spec.html#export-ir-spec)  . 

### Strict vs. Non-Strict Tracing 

[`torch.export.export()`](export.html#torch.export.export "torch.export.export")  provides two modes of tracing. 
...
difference is that all Tensors are replaced by [fake Tensors](https://localhost:8000/docs/main/torch.compiler_fake_tensor.html)  , **which have shapes and other forms of metadata but no data** , wrapped in [Proxy objects](https://localhost:8000/docs/main/fx.html)  that record all
operations on them into a graph. We also capture [conditions on Tensor shapes](https://localhost:8000/docs/main/torch.compiler_dynamic_shapes.html#the-guard-model) **that guard the correctness of the generated code** . 

In *strict mode*  , we first trace through the program using [TorchDynamo](torch.compiler_dynamo_deepdive.html#torch-compiler-dynamo-deepdive)  , a Python bytecode
...
On the one hand, this analysis allows [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  to provide
...
In the rest of this document we assume we are tracing in [non-strict mode](https://localhost:8000/docs/main/export.html#non-strict-export)  ;
in particular, we assume that **all Python features are supported** .

Values: Static vs. Dynamic 
--------------------------------------------------------------------------------------

A key concept in understanding the behavior of [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  is
...
* For Tensor:
    ...
    - By default, shapes of all input Tensors are considered static.
                ...
                a [dynamic shape](https://localhost:8000/docs/main/export.html#expressing-dynamism)  for it.
...
* For Python *standard containers*  ( `list`  , `tuple`  , `dict`  , `namedtuple`  ):
    ...
    + The structure (i.e., length for `list`  and `tuple`  values, and key
        ...
        (basically the [PyTree](https://jax.readthedocs.io/en/latest/pytrees.html)  scheme)
...
For optional inputs to the program that are not passed in, [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  will specialize to their default values. As a
...
Control flow is supported by [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  . The behavior of
...
When the value involved in a control flow is a [dynamic shape](https://localhost:8000/docs/main/torch.compiler_dynamic_shapes.html)  ,
...
concrete value of the dynamic shape to evaluate the condition** to either `True`  or `False`  and continue tracing (as discussed above), additionally
...
Here’s an example of an `if`  statement on a data-dependent condition, `x.sum() > 0`  , where `x`  is an input Tensor, rewritten using `torch.cond`  .
...
A special case of data-dependent control flow is where it involves a [data-dependent dynamic shape](https://localhost:8000/docs/main/torch.compiler_dynamic_shapes.html#unbacked-symints)  :
...
We provide **operators to express assertions on dynamic shapes** , e.g., `torch._check`  . Note that you only need to use this when there is control
...
calling [`torch.nonzero()`](generated/torch.nonzero.html#torch.nonzero "torch.nonzero")  , an operator whose output shape depends on input
data. Instead of rewriting it, you can add an assertion using `torch._check`  to effectively decide which branch to trace. 
...
the output of [`torch.cat()`](generated/torch.cat.html#torch.cat "torch.cat")  is the sum of the shapes of its inputs. 
...
to trace through the program** , and recorded in a [shape environment](https://localhost:8000/docs/main/torch.compiler_dynamic_shapes.html#overall-architecture)  to guard the correctness of the traced path and to evaluate subsequently
created expressions. 
...
Recall that during tracing, we are executing the program with [fake Tensors](https://localhost:8000/docs/main/torch.compiler_fake_tensor.html)  ,
...
For example, note how the fake implementation of [`torch.index_select()`](generated/torch.index_select.html#torch.index_select "torch.index_select")  computes the shape of the output using the shape of the input (while ignoring
...
concrete values of those dynamic shapes to evaluate the condition to `True`  or `False`  . We can then add a guard to the shape environment that states
...
In addition, you can define and use [custom operators](https://localhost:8000/tutorials/advanced/python_custom_ops#python-custom-ops-tutorial)  .
...
data-dependent shapes** . Here’s how a fake implementation for a custom `nonzero`  might look like. 
...
during training, not during inference. We recommend exporting with [`torch.no_grad()`](generated/torch.no_grad.html#torch.no_grad "torch.no_grad")  to avoid parameter updates at export time.
