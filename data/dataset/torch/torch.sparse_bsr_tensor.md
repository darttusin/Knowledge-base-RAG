...
torch. sparse_bsr_tensor ( *crow_indices*  , *col_indices*  , *values*  , *size = None*  , *** , *dtype = None*  , *device = None*  , *pin_memory = False*  , *requires_grad = False*  , *check_invariants = None* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
:   Constructs a [sparse tensor in BSR (Block Compressed Sparse Row))](../sparse.html#sparse-bsr-docs)  with specified 2-dimensional blocks at the given `crow_indices`  and `col_indices`  . Sparse matrix
...
note on the data type of the indices](../sparse.html#sparse-bsr-docs)  . 
...
Parameters
:   * **crow_indices** ( *array_like*  ) – (B+1)-dimensional array of size `(*batchsize, nrowblocks + 1)`  . The last element of each
...
sparse tensor: `(*batchsize, nrows * blocksize[0], ncols * blocksize[1], *densesize)`  where `blocksize == values.shape[1:3]`  . If not provided, the size will be
