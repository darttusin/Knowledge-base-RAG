...
then be optimized and executed by a backend. This can be done today through [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  . 
...
specifically with [`torch.fx`](fx.html#module-torch.fx "torch.fx")  and its related toolings. Thus it will stop
describing contents present in [`torch.fx`](fx.html#module-torch.fx "torch.fx")  documentation and paper.

What is Export IR 
----------------------------------------------------------------------

Export IR is a graph-based intermediate representation IR of PyTorch programs.
Export IR is realized on top of [`torch.fx.Graph`](fx.html#torch.fx.Graph "torch.fx.Graph")  . In other words, **all
...
The top-level Export IR construct is an [`torch.export.ExportedProgram`](export.html#torch.export.ExportedProgram "torch.export.ExportedProgram")  class. It bundles the computational graph of a PyTorch model (which is usually a [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module "torch.nn.Module")  ) with the parameters or weights that this model
consumes. 

Some notable attributes of the [`torch.export.ExportedProgram`](export.html#torch.export.ExportedProgram "torch.export.ExportedProgram")  class are: 

* `graph_module`  ( [`torch.fx.GraphModule`](fx.html#torch.fx.GraphModule "torch.fx.GraphModule")  ): Data structure containing
the flattened computational graph of the PyTorch model. The graph can be
directly accessed through `ExportedProgram.graph`  .
* `graph_signature`  ( [`torch.export.ExportGraphSignature`](export.html#torch.export.ExportGraphSignature "torch.export.ExportGraphSignature")  ): The graph
...
In practice, Export IR’s graph is realized as [`torch.fx.Graph`](fx.html#torch.fx.Graph "torch.fx.Graph")  Python class. 
...
**Definition:** The set of `placeholder`  nodes of a Graph represents the **inputs** of the
Graph of GraphModule. The `output`  node of a Graph represents the **outputs** of the Graph of GraphModule. 
...
Python using the [`torch.fx.Node`](fx.html#torch.fx.Node "torch.fx.Node")  class. Edges between nodes are
...
* is the name of the node as it would appear in `node.name`  .
* **<op_name>** is the `node.op`  field, which must be one of these: `<call_function>`  , `<placeholder>`  , `<get_attr>`  , or `<output>`  .
* is the target of the node as `node.target`  . The meaning of this
field depends on `op_name`  .
* **args1, … args 4…** are what is listed in the `node.args`  tuple. If a
value in the list is an [`torch.fx.Node`](fx.html#torch.fx.Node "torch.fx.Node")  , then it will be especially
...
of type `<symint>`  , `<FakeTensor>`  , a `List[Union[FakeTensor, SymInt]]`  , or `None`  .
* `node.meta["nn_module_stack"]`  describes the “stacktrace” of the [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module "torch.nn.Module")  from which the node came, if it was from a [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module "torch.nn.Module")  call. For example, if a node containing the `addmm`  op called from a [`torch.nn.Linear`](generated/torch.nn.Linear.html#torch.nn.Linear "torch.nn.Linear")  module inside of a [`torch.nn.Sequential`](generated/torch.nn.Sequential.html#torch.nn.Sequential "torch.nn.Sequential")  module, the `nn_module_stack`  would look
something like:
...
* `node.meta["source_fn_stack"]`  contains the torch function or the leaf [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module "torch.nn.Module")  class this node was called from before decomposition.
For example, a node containing the `addmm`  op from a [`torch.nn.Linear`](generated/torch.nn.Linear.html#torch.nn.Linear "torch.nn.Linear")  module call would contain [`torch.nn.Linear`](generated/torch.nn.Linear.html#torch.nn.Linear "torch.nn.Linear")  in
their `source_fn`  , and a node containing the `addmm`  op from a `torch.nn.functional.Linear`  module call would contain `torch.nn.functional.Linear`  in their `source_fn`  .
...
Placeholder nodes also have `meta[‘val’]`  , like `call_function`  nodes. The `val`  field in this case represents the input shape/dtype that the graph is
...
This has the exact semantics as in [`torch.fx`](fx.html#module-torch.fx "torch.fx")  . `args`  represents the node
to be returned. 
...
`get_attr`  nodes represent reading a submodule from the encapsulating [`torch.fx.GraphModule`](fx.html#torch.fx.GraphModule "torch.fx.GraphModule")  . Unlike a vanilla FX graph from [`torch.fx.symbolic_trace()`](fx.html#torch.fx.symbolic_trace "torch.fx.symbolic_trace")  in which `get_attr`  nodes are used to read
attributes such as parameters and buffers from the top-level [`torch.fx.GraphModule`](fx.html#torch.fx.GraphModule "torch.fx.GraphModule")  , parameters and buffers are passed in as
inputs to the graph module, and stored in the top-level [`torch.export.ExportedProgram`](export.html#torch.export.ExportedProgram "torch.export.ExportedProgram")  . 
...
> The concept of pytree is the same as the one documented [here](https://jax.readthedocs.io/en/latest/pytrees.html)  for JAX:

The following types are defined as **leaf type** : 

| Type | Definition |
| --- | --- |
| Tensor | [`torch.Tensor`](tensors.html#torch.Tensor "torch.Tensor") |
...
| bool | Python bool |
| str | Python string |
| ScalarType | [`torch.dtype`](tensor_attributes.html#torch.dtype "torch.dtype") |
| Layout | [`torch.layout`](tensor_attributes.html#torch.layout "torch.layout") |
| MemoryFormat | [`torch.memory_format`](tensor_attributes.html#torch.memory_format "torch.memory_format") |
| Device | [`torch.device`](tensor_attributes.html#torch.device "torch.device") |
...
| Dataclass | Must be registered through [register_dataclass](https://github.com/pytorch/pytorch/blob/901aa85b58e8f490631ce1db44e6555869a31893/torch/export/__init__.py#L693) |
| Custom class | Any custom class defined with [_register_pytree_node](https://github.com/pytorch/pytorch/blob/901aa85b58e8f490631ce1db44e6555869a31893/torch/utils/_pytree.py#L72) |
