...
torch.func. vmap ( *func*  , *in_dims = 0*  , *out_dims = 0*  , *randomness = 'error'*  , *** , *chunk_size = None* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_functorch/apis.py#L30) 
...
vmap is useful for handling batch dimensions: one can write a function `func`  that runs on examples and then lift it to a function that can
take batches of examples with `vmap(func)`  . vmap can also be used to
compute batched gradients when composed with autograd. 

Note 

[`torch.vmap()`](torch.vmap.html#torch.vmap "torch.vmap")  is aliased to [`torch.func.vmap()`](#torch.func.vmap "torch.func.vmap")  for
convenience. Use whichever one you’d like.

Parameters
:   * **func** ( *function*  ) – A Python function that takes one or more arguments.
Must return one or more Tensors.
* **in_dims** ( [*int*](https://docs.python.org/3/library/functions.html#int "(in Python v3.13)") *or* *nested structure*  ) – Specifies which dimension of the
...
Default: 0.
* **out_dims** ( [*int*](https://docs.python.org/3/library/functions.html#int "(in Python v3.13)") *or* *Tuple* *[* [*int*](https://docs.python.org/3/library/functions.html#int "(in Python v3.13)") *]*  ) – Specifies where the mapped dimension
should appear in the outputs. If `out_dims`  is a Tuple, then
it should have one element per output. Default: 0.
* **randomness** ( [*str*](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)")  ) – Specifies whether the randomness in this
...
* **chunk_size** ( *None* *or* [*int*](https://docs.python.org/3/library/functions.html#int "(in Python v3.13)")  ) – If None (default), apply a single vmap over inputs.
...
Returns
:   Returns a new “batched” function. It takes the same inputs as `func`  , except each input has an extra dimension at the index
specified by `in_dims`  . It takes returns the same outputs as `func`  , except each output has an extra dimension at the index
specified by `out_dims`  .

Return type
:   [*Callable*](https://docs.python.org/3/library/typing.html#typing.Callable "(in Python v3.13)")

One example of using [`vmap()`](#torch.func.vmap "torch.func.vmap")  is to compute batched dot products. PyTorch
doesn’t provide a batched `torch.dot`  API; instead of unsuccessfully
rummaging through docs, use [`vmap()`](#torch.func.vmap "torch.func.vmap")  to construct a new function. 
...
[`vmap()`](#torch.func.vmap "torch.func.vmap")  can be helpful in hiding batch dimensions, leading to a simpler
...
[`vmap()`](#torch.func.vmap "torch.func.vmap")  can also help vectorize computations that were previously difficult
or impossible to batch. One example is higher-order gradient computation.
The PyTorch autograd engine computes vjps (vector-Jacobian products).
Computing a full Jacobian matrix for some function f: R^N -> R^N usually
requires N calls to `autograd.grad`  , one per Jacobian row. Using [`vmap()`](#torch.func.vmap "torch.func.vmap")  ,
...
[`vmap()`](#torch.func.vmap "torch.func.vmap")  can also be nested, producing an output with multiple batched dimensions 
