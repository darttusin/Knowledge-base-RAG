...
torch. median ( *input* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
...
compute the mean of both medians, use [`torch.quantile()`](torch.quantile.html#torch.quantile "torch.quantile")  with `q=0.5`  instead.
...
two medians is returned. To compute the mean of both medians in `input`  , use [`torch.quantile()`](torch.quantile.html#torch.quantile "torch.quantile")  with `q=0.5`  instead.
