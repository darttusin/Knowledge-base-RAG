...
Tensor. scatter_ ( *dim*  , *index*  , *src*  , *** , *reduce = None* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
...
This is the reverse operation of the manner described in [`gather()`](torch.Tensor.gather.html#torch.Tensor.gather "torch.Tensor.gather")  . 

`self`  , `index`  and `src`  (if it is a Tensor) should all have
...
Moreover, as for [`gather()`](torch.Tensor.gather.html#torch.Tensor.gather "torch.Tensor.gather")  , the values of `index`  must be
...
Reducing with the addition operation is the same as using [`scatter_add_()`](torch.Tensor.scatter_add_.html#torch.Tensor.scatter_add_ "torch.Tensor.scatter_add_")  . 

Warning 

The reduce argument with Tensor `src`  is deprecated and will be removed in
a future PyTorch release. Please use [`scatter_reduce_()`](torch.Tensor.scatter_reduce_.html#torch.Tensor.scatter_reduce_ "torch.Tensor.scatter_reduce_")  instead for more reduction options.
...
* **src** ( [*Tensor*](../tensors.html#torch.Tensor "torch.Tensor")  ) – the source element(s) to scatter.

Keyword Arguments
: **reduce** ( [*str*](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)") *,* *optional*  ) – reduction operation to apply, can be either `'add'`  or `'multiply'`  .
