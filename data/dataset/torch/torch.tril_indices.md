...
torch. tril_indices ( *row*  , *col*  , *offset = 0*  , *** , *dtype = torch.long*  , *device = 'cpu'*  , *layout = torch.strided* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
:   Returns the indices of the lower triangular part of a `row`  -by- `col`  matrix in a 2-by-N Tensor, where the first row contains row
...
The argument `offset`  controls which diagonal to consider. If `offset`  = 0, all elements on and below the main diagonal are
