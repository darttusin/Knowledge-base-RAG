...
through message flows in different scenarios. Make sure you’re familiar with the [Distributed RPC Framework](../rpc.html#distributed-rpc-framework)  before proceeding. 
...
shared pointer. Applications can create an RRef by calling [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  . Each RRef is owned by the callee worker
of the [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  call (i.e., owner) and can be used
...
which is assigned at the time of creation on the caller of the [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  call. 
...
A `UserRRef`  will be created when it is used as an argument or return value in [`rpc_sync()`](../rpc.html#torch.distributed.rpc.rpc_sync "torch.distributed.rpc.rpc_sync")  , [`rpc_async()`](../rpc.html#torch.distributed.rpc.rpc_async "torch.distributed.rpc.rpc_async")  or [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  invocation, and the owner will be notified
...
training.
* **Non-idempotent UDFs** : We assume the user functions (UDF) provided to [`rpc_sync()`](../rpc.html#torch.distributed.rpc.rpc_sync "torch.distributed.rpc.rpc_sync")  , [`rpc_async()`](../rpc.html#torch.distributed.rpc.rpc_async "torch.distributed.rpc.rpc_async")  or [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  are not idempotent and therefore
...
The right time to delete an `OwnerRRef`  is when there are no living `UserRRef`  instances and user code is not holding references to the `OwnerRRef`  either. The tricky part is to determine if there are any living `UserRRef`  instances. 
...
owner calls [`rpc_sync()`](../rpc.html#torch.distributed.rpc.rpc_sync "torch.distributed.rpc.rpc_sync")  , [`rpc_async()`](../rpc.html#torch.distributed.rpc.rpc_async "torch.distributed.rpc.rpc_async")  , or [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  and uses its RRef as an argument. In this
...
The only requirement is that any `UserRRef`  must notify the owner upon destruction. Hence, we need the first
guarantee: 
...
The owner’s view of the entire tree keeps changing. The owner deletes its `OwnerRRef`  instance when it thinks there are no living `UserRRef`  instances, i.e.,
...
owner at least knows A when Z is deleted, because otherwise, `to_here()`  wouldn’t finish. If Z does not call `to_here()`  , it is possible that the owner
...
**G1** is implemented by sending out a delete message in `UserRRef`  destructor. To provide **G2** , the parent `UserRRef`  is put into a context
...
creates the `OwnerRRef`  . The method [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  returns immediately, meaning that the `UserRRef`  can be forked/used before
the owner knows about it. 

On the owner, when receiving the [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  call, it
will create the `OwnerRRef`  , and returns an ACK to acknowledge `{100, 1}`  ( `RRefId`  , `ForkId`  ). Only after receiving this ACK, can A delete its `UserRRef`  . This involves both **G1** and **G2** . **G1** is obvious. For **G2** , the `OwnerRRef`  is a child of the `UserRRef`  , and the `UserRRef`  is not deleted until it receives the ACK from the owner. 

[![user_to_owner_ret.png](https://user-images.githubusercontent.com/16999635/69164772-98181300-0abe-11ea-93a7-9ad9f757cd94.png)](https://user-images.githubusercontent.com/16999635/69164772-98181300-0abe-11ea-93a7-9ad9f757cd94.png)
...
from A to B ( [`remote()`](../rpc.html#torch.distributed.rpc.remote "torch.distributed.rpc.remote")  and `to_here()`  ) may
...
[![user_to_owner_arg.png](https://user-images.githubusercontent.com/16999635/69164845-b67e0e80-0abe-11ea-93fa-d24674e75a2b.png)](https://user-images.githubusercontent.com/16999635/69164845-b67e0e80-0abe-11ea-93fa-d24674e75a2b.png)

Note that the `UserRRef`  could be deleted on B before func finishes or even
starts. However this is OK, as at the time B sends out ACK for the child `ForkId`  , it already acquired the `OwnerRRef`  instance, which would prevent
...
[![owner_to_user.png](https://user-images.githubusercontent.com/16999635/69164921-c990de80-0abe-11ea-9250-d32ad00cf4ae.png)](https://user-images.githubusercontent.com/16999635/69164921-c990de80-0abe-11ea-9250-d32ad00cf4ae.png)
...
there is a map to hold it alive if there is any known forks, in which case is `UserRRef {100, 1}`  . ( **G2** )
...
[![user_to_user.png](https://user-images.githubusercontent.com/16999635/69164971-d6adcd80-0abe-11ea-971d-6b7af131f0fd.png)](https://user-images.githubusercontent.com/16999635/69164971-d6adcd80-0abe-11ea-971d-6b7af131f0fd.png)
...
provided function. During this time, the parent (A) will hold its `UserRRef {100, 1}`  alive to achieve **G2** .
