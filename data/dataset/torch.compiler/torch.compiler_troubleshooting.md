...
* [Logging Tools](#logging-tools)
    ...
    + [tlparse / TORCH_TRACE](#tlparse-torch-trace)
        + [TORCH_LOGS](#torch-logs)
        + [tlparse vs. TORCH_LOGS](#tlparse-vs-torch-logs)
* [Simple Workarounds](#simple-workarounds)

    + [Where to apply torch.compile?](#where-to-apply-torch-compile)
        + [Disabling and Suppressing Errors](#disabling-and-suppressing-errors)
        + [Resolving graph breaks](#resolving-graph-breaks)

    - [Data-dependent operations](#data-dependent-operations)
                - [Custom ops](#custom-ops)
                - [Printing](#printing)
                - [Incorrect code](#incorrect-code)
        + [Dealing with recompilations](#dealing-with-recompilations)

    - [Is dynamic shapes enabled?](#is-dynamic-shapes-enabled)
                - [Changing the cache size limit](#changing-the-cache-size-limit)
                - [Wrapping constants with tensors](#wrapping-constants-with-tensors)
* [Reporting Issues](#reporting-issues)

    + [Ablation](#ablation)
        + [Bisecting](#bisecting)
        + [Creating a reproducer](#creating-a-reproducer)
        + [Minifier](#minifier)
* [Debugging Deeper](#debugging-deeper)

    + [TorchDynamo](#torchdynamo)

    - [Logging what Dynamo is tracing](#logging-what-dynamo-is-tracing)
                - [Breakpointing Dynamo tracing](#breakpointing-dynamo-tracing)
                - [Bytecode generation errors](#bytecode-generation-errors)
        + [AOTAutograd](#aotautograd)
        + [Summary of TORCH_LOGS options](#summary-of-torch-logs-options)
* [Related Articles](#related-articles)

[Setting Expectations](#id1) 
------------------------------------------------------------------------------------

`torch.compile`  is designed as a general-purpose PyTorch compiler.
Unlike the previous compiler solution, TorchScript, `torch.compile`  requires fewer code changes, meaning models typically don’t need to be rewritten from scratch.
...
### [tlparse / TORCH_TRACE](#id9) 

`tlparse`  / `TORCH_TRACE`  are a pair of tools that produce compilation reports that look like this: [https://web.mit.edu/~ezyang/Public/bhack-20240609-tlparse/index.html](https://web.mit.edu/~ezyang/Public/bhack-20240609-tlparse/index.html)  . 
...
More `TORCH_LOGS`  options are [Summary of TORCH_LOGS options](#troubleshooting-torch-logs-options)  .
For the full list of options, see [torch._logging](https://localhost:8000/docs/stable/logging.html)  and [torch._logging.set_logs](https://localhost:8000/docs/stable/generated/torch._logging.set_logs.html#torch._logging.set_logs)  .

### [tlparse vs. TORCH_LOGS](#id11) 

Generally, we suggest first using `tlparse`  when encountering issues. `tlparse`  is ideal for debugging large models and gaining a high-level overview of how your model was compiled.
...
### [Where to apply torch.compile?](#id13) 
...
### [Disabling and Suppressing Errors](#id14) 
...
By default, all recursive calls made from a disabled function are also disabled. Use the `recursive=False`  option to allow compilation for recursive calls. 
...
If you are experiencing compiler crashes and you want to continue regardless, you can set `torch._dynamo.config.suppress_errors = True`  . When the compiler crashes, we will just skip tracing
...
graph breaks in a preprocessing part at the beginning of the `forward`  . So it is not crucial to prevent *every single*  break, but rather to prevent the ones that cause significant performance hits. 
...
#### [Data-dependent operations](#id16) 
...
* Use higher-order ops like `torch.cond`  ( <https://localhost:8000/docs/main/cond.html> ) in place of data-dependent control flow
...
See <https://localhost:8000/tutorials/advanced/custom_ops_landing_page.html> for more details.
...
#### [Changing the cache size limit](#id22) 

There is a limit to how many times a function can be recompiled, determined by `torch._dynamo.config.recompile_limit`  and `torch._dynamo.config.accumulated_recompile_limit`  .
If either limit is exceeded, then we will not attempt to compile the function again and instead will run the function eagerly. `torch.compile`  will also issue a warning containing the affected function and which limit was hit.
...
#### [Wrapping constants with tensors](#id23) 
...
3. **Non-self-contained reproducer with manageable dependencies:** For example, if you can reproduce the problem by running a script after `pip install transformers`  ,
...
Instructions for operating the minifier can be found [here](https://localhost:8000/docs/stable/torch.compiler_troubleshooting_old.html)  .
If the compiler is crashing, you can set `TORCHDYNAMO_REPRO_AFTER="dynamo"`  or `TORCHDYNAMO_REPRO_AFTER="aot"`  The `aot`  option is more likely to succeed, although it may not identify the `AOTAutograd`  issues. This will generate the `repro.py`  file which may help to diagnose the problem.
...
![Torch Dynamo Stack](_images/td_stack.png)
...
#### [Logging what Dynamo is tracing](#id31) 
...
#### [Breakpointing Dynamo tracing](#id32) 
...
#### [Bytecode generation errors](#id33) 
...
### [Summary of TORCH_LOGS options](#id35) 
...
[Related Articles](#id36) 
-----------------------------------------------------------------------------

* [torch.compile tutorial](https://localhost:8000/tutorials/intermediate/torch_compile_tutorial.html)
* [torch.compile fine-grained APIs](https://localhost:8000/docs/stable/torch.compiler_fine_grain_apis.html)
* [torch.compile FAQ](https://localhost:8000/docs/stable/torch.compiler_faq.html)
* [torch.compiler namespace overview](https://localhost:8000/docs/stable/torch.compiler.html#torch-compiler-overview)
* [torch.compiler API reference](https://localhost:8000/docs/stable/torch.compiler_api.html)
* [Profiling torch.compile](https://localhost:8000/docs/stable/torch.compiler_profiling_torch_compile.html)
...
* [TorchInductor caching tutorial](https://localhost:8000/tutorials/recipes/torch_compile_caching_tutorial.html)
