...
Tensor. sparse_mask ( *mask* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
...
The returned sparse tensor has the same indices as the sparse tensor `mask`  , even when the corresponding values in `self`  are
zeros.

Parameters
: **mask** ( [*Tensor*](../tensors.html#torch.Tensor "torch.Tensor")  ) – a sparse tensor whose indices are used as a filter
