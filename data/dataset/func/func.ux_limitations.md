...
If you are trying to use a `torch.autograd`  API like `torch.autograd.grad`  or `torch.autograd.backward`  inside of a function being transformed by [`vmap()`](generated/torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  or one of torch.func’s AD transforms ( [`vjp()`](generated/torch.func.vjp.html#torch.func.vjp "torch.func.vjp")  , [`jvp()`](generated/torch.func.jvp.html#torch.func.jvp "torch.func.jvp")  , [`jacrev()`](generated/torch.func.jacrev.html#torch.func.jacrev "torch.func.jacrev")  , [`jacfwd()`](generated/torch.func.jacfwd.html#torch.func.jacfwd "torch.func.jacfwd")  ), the transform may not be able to transform over it.
...
[`vmap()`](generated/torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  is our most restrictive transform.
The grad-related transforms ( [`grad()`](generated/torch.func.grad.html#torch.func.grad "torch.func.grad")  , [`vjp()`](generated/torch.func.vjp.html#torch.func.vjp "torch.func.vjp")  , [`jvp()`](generated/torch.func.jvp.html#torch.func.jvp "torch.func.jvp")  ) do not
have these limitations. [`jacfwd()`](generated/torch.func.jacfwd.html#torch.func.jacfwd "torch.func.jacfwd")  (and [`hessian()`](generated/torch.func.hessian.html#torch.func.hessian "torch.func.hessian")  , which is
implemented with [`jacfwd()`](generated/torch.func.jacfwd.html#torch.func.jacfwd "torch.func.jacfwd")  ) is a composition of [`vmap()`](generated/torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  and [`jvp()`](generated/torch.func.jvp.html#torch.func.jvp "torch.func.jvp")  so it also has these limitations.
...
In the presence of side effects, [`vmap()`](generated/torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  no longer acts like it is running
...
[`vmap()`](generated/torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  executes `f`  a single time, so all side effects only happen once. 
...
operations. [`vmap()`](generated/torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  will raise an error if it encounters an unsupported PyTorch
...
`x`  is a Tensor with one element, `y`  is a Tensor with three elements. `x + y`  has three elements (due to broadcasting), but attempting to write
...
There is no problem if the Tensor being written to is batched under [`vmap()`](generated/torch.vmap.html#torch.vmap "torch.vmap")  (i.e. it is being vmapped over). 
...
* Replace [`torch.zeros()`](generated/torch.zeros.html#torch.zeros "torch.zeros")  with `Tensor.new_zeros()`
* Replace [`torch.empty()`](generated/torch.empty.html#torch.empty "torch.empty")  with `Tensor.new_empty()`
...
Inside of [`vmap()`](generated/torch.vmap.html#torch.vmap "torch.vmap")  , `result`  is a Tensor of shape [3, 3].
...
Replacing [`torch.zeros()`](generated/torch.zeros.html#torch.zeros "torch.zeros")  with `Tensor.new_zeros()`  makes it so that `result`  has an underlying Tensor of shape [2, 3, 3], so it is now possible
to copy `vec`  , which has underlying shape [2, 3], into `result.diagonal()`  .

### Mutation: out= PyTorch Operations 

[`vmap()`](generated/torch.func.vmap.html#torch.func.vmap "torch.func.vmap")  doesn’t support the `out=`  keyword argument in PyTorch operations.
...
JAX supports transforming over [data-dependent control flow](https://jax.readthedocs.io/en/latest/jax.lax.html#control-flow-operators)  using special control flow operators (e.g. `jax.lax.cond`  , `jax.lax.while_loop`  ).
...
returns a Tensor with the same shape. Operations such as `torch.nonzero`  , `torch.is_nonzero`  are not supported and will error as a result. 
