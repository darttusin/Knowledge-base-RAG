...
torch.fft. fft2 ( *input*  , *s = None*  , *dim = (-2, -1)*  , *norm = None*  , *** , *out = None* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
:   Computes the 2 dimensional discrete Fourier transform of `input`  .
Equivalent to [`fftn()`](torch.fft.fftn.html#torch.fft.fftn "torch.fft.fftn")  but FFTs only the last two dimensions by default. 
...
though, for real inputs, half of these values are redundant. [`rfft2()`](torch.fft.rfft2.html#torch.fft.rfft2 "torch.fft.rfft2")  returns the more compact one-sided representation
...
* **norm** ( [*str*](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.13)") *,* *optional*  ) –
    ...
    Normalization mode. For the forward transform
        ( [`fft2()`](#torch.fft.fft2 "torch.fft.fft2")  ), these correspond to:

    + `"forward"`  - normalize by `1/n`
        + `"backward"`  - no normalization
        + `"ortho"`  - normalize by `1/sqrt(n)`  (making the FFT orthonormal)
        Where `n = prod(s)`  is the logical FFT size.
        Calling the backward transform ( [`ifft2()`](torch.fft.ifft2.html#torch.fft.ifft2 "torch.fft.ifft2")  ) with the same
        normalization mode will apply an overall normalization of `1/n`  between the two transforms. This is required to make [`ifft2()`](torch.fft.ifft2.html#torch.fft.ifft2 "torch.fft.ifft2")  the exact inverse.
...
The discrete Fourier transform is separable, so [`fft2()`](#torch.fft.fft2 "torch.fft.fft2")  here is equivalent to two one-dimensional [`fft()`](torch.fft.fft.html#torch.fft.fft "torch.fft.fft")  calls: 
