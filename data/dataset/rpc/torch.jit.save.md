...
torch.jit. save ( *m*  , *f*  , *_extra_files = None* ) [source](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/jit/_serialization.py#L21) 
...
attributes of this module. It can be loaded into the C++ API using `torch::jit::load(filename)`  or into the Python API with [`torch.jit.load`](torch.jit.load.html#torch.jit.load "torch.jit.load")  . 

To be able to save a module, it must not make any calls to native Python
functions. This means that all submodules must be subclasses of [`ScriptModule`](torch.jit.ScriptModule.html#torch.jit.ScriptModule "torch.jit.ScriptModule")  as well. 

Danger 

All modules, no matter their device, are always loaded onto the CPU
during loading. This is different from [`torch.load()`](torch.load.html#torch.load "torch.load")  ’s semantics
and may change in the future.

Parameters
:   * **m** – A [`ScriptModule`](torch.jit.ScriptModule.html#torch.jit.ScriptModule "torch.jit.ScriptModule")  to save.
