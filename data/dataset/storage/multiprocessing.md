...
[`torch.multiprocessing`](../multiprocessing.html#module-torch.multiprocessing "torch.multiprocessing")  is a drop in replacement for Python’s [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing "(in Python v3.13)")  module. It supports the exact same operations,
but extends it, so that all tensors sent through a [`multiprocessing.Queue`](https://docs.python.org/3/library/multiprocessing.html#multiprocessing.Queue "(in Python v3.13)")  , will have their data moved into shared
memory and will only send a handle to another process. 

Note 

When a [`Tensor`](../tensors.html#torch.Tensor "torch.Tensor")  is sent to another process, the [`Tensor`](../tensors.html#torch.Tensor "torch.Tensor")  data is shared. If [`torch.Tensor.grad`](../generated/torch.Tensor.grad.html#torch.Tensor.grad "torch.Tensor.grad")  is
not `None`  , it is also shared. After a [`Tensor`](../tensors.html#torch.Tensor "torch.Tensor")  without
a [`torch.Tensor.grad`](../generated/torch.Tensor.grad.html#torch.Tensor.grad "torch.Tensor.grad")  field is sent to the other process, it
creates a standard process-specific `.grad` [`Tensor`](../tensors.html#torch.Tensor "torch.Tensor")  that
is not automatically shared across all processes, unlike how the [`Tensor`](../tensors.html#torch.Tensor "torch.Tensor")  ’s data has been shared.
...
When using multiprocessing with [accelerators](../torch.html#accelerators)  , a known issue called “poison fork” may occur.
...
The CUDA runtime has the limitation described in [Poison fork in multiprocessing](#multiprocessing-poison-fork-note)  when using the `fork`  start method;
...
save you if the consumer process exits abnormally via a fatal signal. See [this section](../multiprocessing.html#multiprocessing-cuda-sharing-details)  . 

See also: [Use nn.parallel.DistributedDataParallel instead of multiprocessing or nn.DataParallel](cuda.html#cuda-nn-ddp-instead)
...
libraries do - no need to look further than [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing "(in Python v3.13)")  . [`multiprocessing.Queue`](https://docs.python.org/3/library/multiprocessing.html#multiprocessing.Queue "(in Python v3.13)")  is actually a very complex class, that
...
Remember that each time you put a [`Tensor`](../tensors.html#torch.Tensor "torch.Tensor")  into a [`multiprocessing.Queue`](https://docs.python.org/3/library/multiprocessing.html#multiprocessing.Queue "(in Python v3.13)")  , it has to be moved into shared memory.
...
Using [`torch.multiprocessing`](../multiprocessing.html#module-torch.multiprocessing "torch.multiprocessing")  , it is possible to train a model
...
model object, while in the latter, we advise to only send the [`state_dict()`](../generated/torch.nn.Module.html#torch.nn.Module.state_dict "torch.nn.Module.state_dict")  . 

We recommend using [`multiprocessing.Queue`](https://docs.python.org/3/library/multiprocessing.html#multiprocessing.Queue "(in Python v3.13)")  for passing all kinds
...
with an `if __name__ == '__main__'`  . If a different start method than `fork`  is used, they will be executed in all subprocesses.
...
of threads for each process using the `torch.set_num_threads(int)`  function in subprocess. 
...
Set `num_thread`  for each process using `torch.set_num_threads(floor(N/M))`  . where you replace N with the
