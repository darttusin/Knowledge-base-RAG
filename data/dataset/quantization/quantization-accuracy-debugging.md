...
weight quantization.
2. If you are doing inference on `fbgemm`  , ensure that you set the `reduce_range`  argument to `False`  if your CPU is Cooperlake or newer, and to `True`  otherwise.
...
* [torch.ao.ns._numeric_suite](torch.ao.ns._numeric_suite.html#torch-ao-ns-numeric-suite)  Eager mode numeric suite
* [torch.ao.ns._numeric_suite_fx](torch.ao.ns._numeric_suite_fx.html#torch-ao-ns-numeric-suite-fx)  FX numeric suite
