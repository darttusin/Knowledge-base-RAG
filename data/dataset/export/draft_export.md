...
Have you ever tried to export a model using [`torch.export.export()`](export.html#torch.export.export "torch.export.export")  , only to
...
defined for it, then we will error with `fake_tensor.UnsupportedOperatorException`  because export doesn’t know how to apply this on `FakeTensors`  . If a custom
...
This runs into a “missing fake kernel” error for `mylib.foo2`  and then a `GuardOnDataDependentExpression`  because of the slicing of `z`  with `a`  ,
an unbacked symint. 
...
errors by using `print(ep._report)`  , or you can pass the logs into `tlparse`  to generate an html report. 

Running the `tlparse`  command in the terminal will generate a [tlparse](https://github.com/pytorch/tlparse)  HTML report. Here is an example of the `tlparse`  report: 

![_images/draft_export_report.png](_images/draft_export_report.png)
...
![_images/draft_export_report_dde.png](_images/draft_export_report_dde.png)
...
draft-export our example inputs were greater than 5, then the returned `ExportedProgram`  will specialize on that branch, and will assert that the value
...
* [AOTInductor: Ahead-Of-Time Compilation for Torch.Export-ed Models](torch.compiler_aot_inductor.html)
