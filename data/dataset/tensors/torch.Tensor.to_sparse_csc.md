...
Tensor. to_sparse_csc ( ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
:   Convert a tensor to compressed column storage (CSC) format. Except
for strided tensors, only works with 2D tensors. If the `self`  is strided, then the number of dense dimensions could be specified,
...
resulting CSC tensor. This argument should be used only if `self`  is a strided tensor, and must be a value between 0
