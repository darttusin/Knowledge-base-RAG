...
torch. tensor ( *data*  , *** , *dtype = None*  , *device = None*  , *requires_grad = False*  , *pin_memory = False* ) → [Tensor](../tensors.html#torch.Tensor "torch.Tensor") 
...
readability. Letting *t* be a tensor, `torch.tensor(t)`  is equivalent to `t.detach().clone()`  , and `torch.tensor(t, requires_grad=True)`  is equivalent to `t.detach().clone().requires_grad_(True)`  .

See also 

[`torch.as_tensor()`](torch.as_tensor.html#torch.as_tensor "torch.as_tensor")  preserves autograd history and avoids copies where possible. [`torch.from_numpy()`](torch.from_numpy.html#torch.from_numpy "torch.from_numpy")  creates a tensor that shares storage with a NumPy array.
