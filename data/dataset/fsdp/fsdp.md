...
model that does not satisfy this constraint is [FLAVA](https://localhost:8000/blog/scaling-multimodal-foundation-models-in-torchmultimodal-with-pytorch-distributed/)  ). 
...
If activation checkpointing ( [`checkpoint()`](../checkpoint.html#torch.utils.checkpoint.checkpoint "torch.utils.checkpoint.checkpoint")  ) is used there is no
...
comprised of 10 transformer blocks there would be a total of 30 communications for a total of `30*0.8=24GB`  . 
...
As explained in [FSDP Prefetch Nuances](#fsdp-prefetch)  in the case of explicit `forward`  prefetching
